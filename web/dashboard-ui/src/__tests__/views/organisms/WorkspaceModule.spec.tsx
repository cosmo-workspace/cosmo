...
} from "../../../proto/gen/dashboard/v1alpha1/workspace_service_pb";
import {
  useTemplateService,
  useUserService,
  useWorkspaceService,
...
import {
  WorkspaceContext,
  WorkspaceUsersContext,
  computeStatus,
  useNetworkRule,
  useTemplates,
  useWorkspaceModule,
  useWorkspaceUsersModule,
} from "../../../views/organisms/WorkspaceModule";
...
const useWorkspaceServiceMock = useWorkspaceService as MockedFunction<
  typeof useWorkspaceService
>;
...
function wsStat(ws: Workspace, replicas: number, phase: string): Workspace {
  return new Workspace({
    ...ws,
    spec: { ...ws.spec!, replicas: protoInt64.parse(replicas) },
    status: { ...ws.status, phase },
  });
}
...
const tmpl2: Template = new Template({
  name: "tmpl2",
  requiredVars: [{ varName: "var1" }, { varName: "var2" }],
});
...
describe("computeStatus", () => {
  const wsStarting = new Workspace({
    name: "",
    ownerName: "",
    spec: { template: "", replicas: protoInt64.parse(1) },
    status: { phase: "Stopped" },
  });
  const wsPending = new Workspace({
    name: "",
    ownerName: "",
    spec: { template: "", replicas: protoInt64.parse(-1) },
    status: { phase: "Pending" },
  });
  const wsStoping = new Workspace({
    name: "",
    ownerName: "",
    spec: { template: "", replicas: protoInt64.parse(0) },
    status: { phase: "Running" },
  });
  const wsStopped = new Workspace({
    name: "",
    ownerName: "",
    spec: { template: "", replicas: protoInt64.parse(0) },
    status: { phase: "Stopped" },
  });
  const wsRunning = new Workspace({
    name: "",
    ownerName: "",
    spec: { template: "", replicas: protoInt64.parse(1) },
    status: { phase: "Running" },
  ...
  it("Stopping", () => {
    expect(computeStatus(wsStoping)).toEqual("Stopping");
  });
  it("Stopped", () => {
    expect(computeStatus(wsStopped)).toEqual("Stopped");
  });
  it("Starting", () => {
    expect(computeStatus(wsStarting)).toEqual("Starting");
  });
  it("Running", () => {
    expect(computeStatus(wsRunning)).toEqual("Running");
  });
  it("Other", async () => {
    expect(computeStatus(wsPending)).toEqual("Pending");
  });
});
...
describe("useWorkspace", () => {
  beforeEach(async () => {
    ...
    wsMock.getWorkspaces.mockResolvedValue(
      new GetWorkspacesResponse({ message: "", items: [ws11, ws13, ws12] })
    );
  });
  ...
  function renderUseWorkspaceModule() {
    return renderHook(() => useWorkspaceModule(), {
      wrapper: ({ children }) => (
        <WorkspaceContext.Provider>{children}</WorkspaceContext.Provider>
      ),
    });
  }
  ...
  describe("useWorkspace getWorkspaces", () => {
    it("normal", async () => {
      ...
      await act(async () => {
        result.current.getWorkspaces(user1.name);
      });
    ...
    it("normal empty", async () => {
      const { result } = renderUseWorkspaceModule();
      wsMock.getWorkspaces.mockResolvedValue(
        new GetWorkspacesResponse({ message: "", items: [] })
      );
      await act(async () => {
        result.current.getWorkspaces(user1.name);
      });
    ...
    it("get error", async () => {
      ...
      wsMock.getWorkspaces.mockRejectedValue(
        new ConnectError("[mock] getWorkspaces error", Code.Unauthenticated)
      );
      await expect(
        result.current.getWorkspaces(user1.name)
  ...
  describe("useWorkspace refreshWorkspaces", () => {
    it("normal", async () => {
      const { result } = renderUseWorkspaceModule();
      await act(async () => {
        result.current.refreshWorkspaces(user1.name);
      });
      expect(result.current.workspaces).toMatchSnapshot();
    });
  });
  ...
  describe("useWorkspace refreshWorkspace", () => {
    it("normal creating", async () => {
      ...
      wsMock.getWorkspaces.mockResolvedValueOnce(
        new GetWorkspacesResponse({
          message: "",
          items: [ws11, wsCreateing, ws13],
        })
      );
      await act(async () => {
        result.current.getWorkspaces(user1.name);
      ...
      // refReshWorkspace
      await act(async () => {
        result.current.refreshWorkspace(wsCreateing);
      });
      ...
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsStarting })
      );
      await act(async () => {
        vi.runAllTimers();
      });
      ...
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsPending })
      );
      await act(async () => {
        vi.runAllTimers();
      });
      ...
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsRunning })
      );
      await act(async () => {
        vi.runAllTimers();
      });
    ...
    it("normal stopping", async () => {
      ...
      // refReshWorkspace
      await act(async () => {
        result.current.refreshWorkspace(wsStoping);
      });

      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsStoping })
      );
      await act(async () => {
      ...
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsStopped })
      );
      await act(async () => {
        vi.runOnlyPendingTimers();
      });
    ...
    it("normal network modify", async () => {
      ...
      // refReshWorkspace
      await act(async () => {
        result.current.refreshWorkspace(wsRunning1);
      });

      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsRunning1 })
      );
      await act(async () => {
        vi.runAllTimers();
      });
      ...
      //　　refReshWorkspace starting
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: wsRunning2 })
      );
      await act(async () => {
        vi.runAllTimers();
      });
    ...
    it("normal timeout", async () => {
      ...
      wsMock.getWorkspace.mockResolvedValue(
        new GetWorkspaceResponse({ workspace: wsStoping })
      );
  ...
  it("error", async () => {
    ...
    wsMock.getWorkspace.mockRejectedValueOnce(
      new Error("[mock] getWorkspace error")
    );
    await act(async () => {
      vi.runOnlyPendingTimers();
    });
  ...
  describe("useWorkspace createWorkspace", () => {
    it("normal", async () => {
      ...
      wsMock.createWorkspace.mockResolvedValueOnce(
        new CreateWorkspaceResponse({ message: "ok", workspace: ws14 })
      );
      await act(async () => {
    ...
    it("get error", async () => {
      const { result } = renderUseWorkspaceModule();
      wsMock.createWorkspace.mockRejectedValue(
        new Error("[mock] createWorkspace error")
      );
      await expect(
        result.current.createWorkspace(
          ws14.ownerName,
  ...
  describe("useWorkspace runWorkspace", () => {
    it("normal", async () => {
      ...
      const { result } = renderUseWorkspaceModule();
      wsMock.getWorkspaces.mockResolvedValueOnce(
        new GetWorkspacesResponse({ message: "", items: [ws11, ws12, ws13] })
      );
      await act(async () => {
        result.current.getWorkspaces(user1.name);
      });
      ...
      wsMock.updateWorkspace.mockResolvedValueOnce(
        new UpdateWorkspaceResponse({ message: "ok", workspace: ws11 })
      );
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: ws11 })
      );
      await act(async () => {
        result.current.runWorkspace(ws11);
      });
      expect(result.current.workspaces).toMatchSnapshot();
    });
    ...
    it("error", async () => {
      const { result } = renderUseWorkspaceModule();
      wsMock.updateWorkspace.mockRejectedValue(
        new Error("[mock] updateWorkspace error")
      );
  ...
  describe("useWorkspace stopWorkspace", () => {
    it("normal", async () => {
      ...
      await act(async () => {
        result.current.stopWorkspace(ws11);
      });
      expect(result.current.workspaces).toMatchSnapshot();
    });
    ...
    it("error", async () => {
      const { result } = renderUseWorkspaceModule();
      wsMock.updateWorkspace.mockRejectedValue(
        new Error("[mock] patchWorkspace error")
      );
      await expect(
        result.current.stopWorkspace(ws11)
      ).rejects.toMatchSnapshot();
  ...
  describe("useWorkspace deleteWorkspace", () => {
    it("normal", async () => {
      ...
      wsMock.deleteWorkspace.mockResolvedValueOnce(
        new DeleteWorkspaceResponse({ message: "ok", workspace: ws11 })
      );
      wsMock.getWorkspace.mockResolvedValueOnce(
        new GetWorkspaceResponse({ workspace: ws11 })
      );
      await act(async () => {
        result.current.deleteWorkspace(ws11);
      });
      expect(result.current.workspaces).toMatchSnapshot();
    });
    ...
    it("error", async () => {
      const { result } = renderUseWorkspaceModule();
      wsMock.deleteWorkspace.mockRejectedValue(
...
describe("useTemplates", () => {
  ...
  it("normal", async () => {
    ...
    templateMock.getWorkspaceTemplates.mockResolvedValue(
      new GetWorkspaceTemplatesResponse({
        message: "",
  ...
  it("normal empty", async () => {
    const { result } = renderHook(() => useTemplates());
    templateMock.getWorkspaceTemplates.mockResolvedValue(
      new GetWorkspaceTemplatesResponse({ message: "", items: [] })
    );
    await act(async () => {
      result.current.getTemplates();
    });
  ...
  it("get error", async () => {
    const { result } = renderHook(() => useTemplates());
    templateMock.getWorkspaceTemplates.mockRejectedValue(
      new Error("[mock] getWorkspaceTemplates error")
    );
...
describe("useNetworkRule", () => {
  ...
  describe("useNetworkRule upsertNetwork", () => {
    it("normal", async () => {
      ...
      wsMock.upsertNetworkRule.mockResolvedValue(
        new UpsertNetworkRuleResponse({ message: "ok", networkRule: nw111 })
      );
      await act(async () => {
        result.current.upsertNetwork(ws11, nw111);
      });
    });

    it("error", async () => {
      const { result } = renderUseNetworkRule();
      wsMock.upsertNetworkRule.mockRejectedValue(
        new Error("[mock] upsertNetworkRule error")
      );
      await expect(
        result.current.upsertNetwork(ws11, nw111)
      ).rejects.toMatchSnapshot();
  ...
  describe("useNetworkRule removeNetwork", () => {
    it("normal", async () => {
      const { result } = renderUseNetworkRule();
      wsMock.deleteNetworkRule.mockResolvedValue(
        new DeleteNetworkRuleResponse({ message: "ok", networkRule: nw111 })
      );
      await act(async () => {
        result.current.removeNetwork(ws11, nw111.name);
      });
    });

    it("error", async () => {
      const { result } = renderUseNetworkRule();
      wsMock.deleteNetworkRule.mockRejectedValue(
        new Error("[mock] deleteNetworkRule error")
      );
      await expect(
        result.current.removeNetwork(ws11, nw111.name)
      ).rejects.toMatchSnapshot();
  ...
  describe("useWorkspaceUsers getUsers", () => {
    ...
    it("normal", async () => {
      ...
      userMock.getUsers.mockResolvedValue(
        new GetUsersResponse({ message: "ok", items: [user2, user1, user3] })
      );
      await act(async () => {
    ...
    it("normal empty", async () => {
      const { result } = renderUseWorkspaceUsersModule();
      userMock.getUsers.mockResolvedValue(
        new GetUsersResponse({ message: "ok", items: [] })
      );
      await act(async () => {
        result.current.getUsers();
      });
