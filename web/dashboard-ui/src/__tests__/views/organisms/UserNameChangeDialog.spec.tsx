...
import {
  ...
  UserNameChangeDialogContext,
} from "../../../views/organisms/UserNameChangeDialog";
...
describe("UserNameChangeDialog", () => {
  ...
  describe("behavior", () => {
    ...
    it("ok", async () => {
      ...
      await user.type(nameElement, "New Name", {
        initialSelectionStart: 0,
        initialSelectionEnd: 99,
      });

      userModuleMock.updateName.mockResolvedValue({} as any);
      loginMock.refreshUserInfo.mockResolvedValue({} as any);
      await user.click(screen.getByText("Update"));
      expect(userModuleMock.updateName.mock.calls).toMatchObject([
        ["user1", "New Name"],
      ]);
    ...
    it("ng required", async () => {
      ...
      expect(
        screen
          .getAllByText("User Name")[0]
          .parentElement!.getElementsByClassName("MuiFormHelperText-root")[0]
      ).toBeUndefined();

      const nameElement = baseElement.querySelector('[name="name"]')!;
      await user.type(nameElement, "{backspace}", {
        initialSelectionStart: 0,
        initialSelectionEnd: 99,
      });
      await act(async () => {
    ...
    it("ng over 32", async () => {
      ...
      const nameElement = baseElement.querySelector('[name="name"]')!;
      await user.type(nameElement, "----+----1----+----2----+----3--x", {
        initialSelectionStart: 0,
        initialSelectionEnd: 99,
      });
      await act(async () => {
      ...
      expect(
        screen
          .getAllByText("User Name")[0]
          .parentElement!.getElementsByClassName("MuiFormHelperText-root")[0]
      ).toHaveTextContent("Max 32 characters");
      expect(userModuleMock.updateName.mock.calls.length).toEqual(0);
      expect(loginMock.refreshUserInfo.mock.calls.length).toEqual(0);
      expect(closeHandlerMock.mock.calls.length).toEqual(0);

      await act(async () => {
        await user.type(nameElement, "----+----1----+----2----+----3--", {
          initialSelectionStart: 0,
          initialSelectionEnd: 99,
        });
      });
    ...
    it("not close <- click outside", async () => {
      ...
      await user.click(
        baseElement.getElementsByClassName("MuiDialog-container")[0]
      );
      expect(userModuleMock.updateName.mock.calls.length).toEqual(0);
      expect(loginMock.refreshUserInfo.mock.calls.length).toEqual(0);
      expect(closeHandlerMock.mock.calls.length).toEqual(0);
  ...
  describe("UserNameChangeDialogContext", () => {
    it("open/close", async () => {
      const Stub = () => {
        ...
        return (
          <>
            <Button onClick={() => dispatch(true, { user: user1 })}>
              open
            </Button>
      ...
      await act(async () => {
        expect(document.body).toMatchSnapshot("1.initial render");
      });
      await act(async () => {
        await user.click(screen.getByText("open"));
      });
      await act(async () => {
        expect(document.body).toMatchSnapshot("2.opend");
      });
      await act(async () => {
        await user.click(screen.getByText("close"));
      });
      await act(async () => {
        expect(document.body).toMatchSnapshot("3.closed");
      });
    });
  });
});
