...
describe("DialogContext", () => {
  ...
  const TestDialog: React.VFC<{ onClose: () => void; hoge: string }> = ({
    onClose,
    hoge,
  }) => {
    return <Dialog open={true} onClose={() => onClose()}></Dialog>;
  };

  const testDialogContext = DialogContext<{ hoge: string }>((props) => (
    <TestDialog {...props} />
  ));
...
describe("ModuleContext", () => {
  it("normal", async () => {
    ...
    const Component1: React.FC<React.PropsWithChildren<unknown>> = ({
      children,
    }) => {
    ...
    const Component2: React.FC<React.PropsWithChildren<unknown>> = ({
      children,
    }) => {
      const { state, setState } = useTest();
      return (
