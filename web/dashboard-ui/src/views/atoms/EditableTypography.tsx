...
export type EditableTypographyProps = InputBaseProps & {
  children: string;
  onSave: (inputData: string) => void;
  showAlways?: boolean;
};
...
export const EditableTypography: React.FC<EditableTypographyProps> = ({
  ...
  return (
    <InputBase
      ...
      placeholder="Name"
      endAdornment={
        editting ? editingIcons : showEditIcon ? editIcon : undefined
      }
      defaultValue={children}
      readOnly={!editting}
      autoFocus={editting}
      onMouseOver={() => {
        setShowEditIcon(showAlways || true);
      }}
      onMouseLeave={() => {
        setShowEditIcon(showAlways || false);
      }}
      onBlur={(e) => {
