import { AddTwoTone, CheckCircleOutlined, Clear, DeleteTwoTone, EditTwoTone, ErrorOutline, ExpandLessTwoTone, ExpandMoreTwoTone, MoreVertTwoTone, OpenInNewTwoTone, PlayCircleFilledWhiteTwoTone, RefreshTwoTone, SearchTwoTone, StopCircleOutlined, StopCircleTwoTone, WebTwoTone } from '@mui/icons-material';
...
const NetworkRuleItem: React.VFC<{ workspace: Workspace, networkRule: NetworkRule }> = ({ workspace, networkRule }) => {
  ...
  return (<>
    <Grid item xs={2} sm={2} md={2} sx={{ m: 'auto' }}>{Body2(networkRule.portName)}</Grid>
    <Grid item xs={2} sm={2} md={1} sx={{ m: 'auto' }}>{Body2(networkRule.portNumber)}</Grid>
    <Grid item xs={2} sm={2} md={2} sx={{ m: 'auto' }}>{Body2(networkRule.group)}</Grid>
    <Grid item xs={3} sm={4} md={6} zeroMinWidth sx={{ m: 'auto' }}>
      <Link href={networkRule.url || ''} target='_blank'>
        <Typography variant='body2' noWrap={!isUpSM} >{networkRule.url || ''} <OpenInNewTwoTone fontSize='inherit' sx={{ position: 'relative', top: '0.2em' }} /></Typography>
      </Link>
    </Grid>
    <Grid item xs={3} sm={2} md={1} sx={{ m: 'auto', textAlign: 'center' }}>
      <Stack direction='row' alignItems='center' justifyContent='center' spacing={{ xs: 0, sm: 1 }} >
        <IconButton
          onClick={() => {
            upsertDialogDispatch(true, { workspace: workspace, networkRule: networkRule });
          }}>
          <EditTwoTone />
        </IconButton>
        <IconButton
          onClick={() => {
            deleteDialogDispatch(true, { workspace: workspace, networkRule: networkRule });
          }}>
          <DeleteTwoTone />
        </IconButton>
      </Stack>
...
const NetworkRuleList: React.VFC<{ workspace: Workspace }> = ({ workspace }) => {
  ...
  return (<>
    <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 2 }}>
      ...
      {/* network rule header */}
      <Grid item xs={2} sm={2} md={2} sx={{ m: 'auto' }}>{Caption('Port Name')}</Grid>
      <Grid item xs={2} sm={2} md={1} sx={{ m: 'auto' }}>{Caption('Port #')}</Grid>
      <Grid item xs={2} sm={2} md={2} sx={{ m: 'auto' }}>{Caption('Group')}</Grid>
      {isUpSM ?
        <Grid item xs={3} sm={4} md={6} zeroMinWidth sx={{ m: 'auto' }}>
          <Stack>{Caption('URL')}{SubCaption('URLBase: ' + workspace.status?.urlBase)}</Stack>
        </Grid>
        :
        <Grid item xs={3} sm={4} md={6} zeroMinWidth sx={{ m: 'auto' }}>{Caption('URL')}</Grid>
      }
      <Grid item xs={3} sm={2} md={1} sx={{ m: 'auto', textAlign: 'center' }}>
        <IconButton
          onClick={() => {
            upsertDialogDispatch(true, { workspace: workspace });
          }}><AddTwoTone /></IconButton>
      </Grid>
      <Grid item xs={12}><Divider /></Grid>
      {/* network rule detail */}
