...
export const isValidRedirectURLDomain = (url: string) => {
  ...
  return (
    extractDomainFromHostname(redirect_hostname) ===
    extractDomainFromHostname(current_hostname)
  );
};
...
const SignInContent: React.VFC = () => {
  ...
  const onSignIn = async (data: Inputs) => {
    ...
    if (usePasswordLogin) {
      const { requirePasswordUpdate } = await login(
        data.username,
        data.password
      );
      ...
      <form noValidate onSubmit={handleSubmit(onSignIn)}>
          ...
          {...registerMui(
            register("username", {
              ...
              pattern: {
                ...
                message:
                  'Only lowercase alphanumeric characters or in "-" are allowed',
              },
        ...
        {usePasswordLogin && (
          <PasswordTextField
            ...
            defaultValue={loginUser && "you-are-logined"}
            {...registerMui(
              register("password", {
                required: { value: true, message: "Required" },
                maxLength: { value: 128, message: "Max 128 characters" },
        ...
        <Button
          ...
          sx={{ mt: 3, mb: 2 }}
        >
          {usePasswordLogin ? "Sign In" : <Fingerprint />}
        </Button>
        <Divider />
        {usePasswordLogin || (
          <Button
