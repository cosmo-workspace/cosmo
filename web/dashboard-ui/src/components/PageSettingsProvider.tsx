...
> = ({ children }) => {
  ...
  const persistPageSettings = JSON.parse(
    localStorage.getItem("csm_PageSettings") || "{}"
  );
  const [pageSettings, dispatch] = useReducer(
    reducer,
    persistPageSettings || {}
  );
    ...
    <Context.Provider
      value={{
        ...
        setPageSettings: (settings) => {
          dispatch({ type: "SET", pageSettings: settings });
        },
        updatePageSettings: (settings) => {
          dispatch({ type: "UPDATE", pageSettings: settings });
        },
      }}
    >
      {children}
