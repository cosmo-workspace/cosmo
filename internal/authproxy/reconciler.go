...
func (r *NetworkRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for i, netRule := range ws.Spec.Network {
		...
		usingProxyList = append(usingProxyList, netRule.NetworkRuleName)

		runningProxyInfo, exist := r.ProxyManager.GetRunningProxy(netRule.NetworkRuleName)
		if exist {
			...
			err := r.ProxyManager.ShutdownProxy(ctx, netRule.NetworkRuleName)
			if err != nil {
				...
				r.Recorder.Eventf(&ws, corev1.EventTypeNormal, "Proxy removed", "successfully shotdown proxy: name=%s portNumber=%d proxyPort=%d",
					netRule.NetworkRuleName, netRule.PortNumber, runningProxyInfo.LocalPort)
			}
		}

		log.Info("creating new proxy", "name", netRule.NetworkRuleName, "targetPort", netRule.TargetPortNumber)

		proxyCreateCtx, cancel := context.WithTimeout(ctx, time.Second*10)
		proxyInfo, err := r.ProxyManager.CreateNewProxy(proxyCreateCtx, netRule.NetworkRuleName, netRule.PortNumber)
		cancel()
		if err != nil {
			r.Recorder.Eventf(&ws, corev1.EventTypeWarning, "Proxy create failed", "failed to create new proxy: %s proxyPort: %d targetPort: %d %v", netRule.NetworkRuleName, proxyInfo.LocalPort, netRule.PortNumber, err.Error())
			continue
		}
		...
		r.Recorder.Eventf(&ws, corev1.EventTypeNormal, "Proxy created", "successfully created new proxy: name=%s portNumber=%d proxyPort=%d",
			netRule.NetworkRuleName, netRule.PortNumber, proxyInfo.LocalPort)
	}
